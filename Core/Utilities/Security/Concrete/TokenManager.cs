...
namespace Core.Utilities.Security.Concrete
{
    public class TokenManager : ITokenService
    {
        ...
        public async Task<Token> CreateAccessTokenAsinc(AppUser appUser, List<string> roles)
        {
            Token token = new();
            var claims = new List<Claim>()
    {
        ...
        new Claim(ClaimTypes.NameIdentifier, appUser.Id.ToString())
            ...
            var result = await _userManager.AddClaimsAsync(appUser, claims);
            if (!result.Succeeded)
            {
                throw new Exception("Failed to add claims to the user.");
            }
